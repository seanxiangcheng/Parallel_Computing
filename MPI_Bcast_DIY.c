 ...
 * a self-coded Broadcast funciton using Send and Receive
 * and compare to MPI_Bcast_Eval.c
*/
...
int main(int argc, char** argv) {
  int rank, i, steps, mysender, proc_size, msg_size[] ={1, 1024, 4096, 65536, 1048576}, size_seq = 0, repeat = 0, tag = 0;
  double t[SIZE_NUM], t1 = 0.0, t2 = 0.0, *all_times, *time_all_sizes, total_t = 0.0;
  char buf[MAX_MSG_SIZE]; 
  const int root=0;
  char filename[50] = "lab0z_sr_n", file_proc[8];
  ...
  strcat(filename, ".txt");
  steps = (int)ceil(log2f((float)proc_size));
  if(rank != root)  mysender = rank % Intpow(2,((int)floor(log2f((float)rank))));
  ...
  for(size_seq = 0; size_seq < SIZE_NUM; size_seq++){
    for(repeat = 0; repeat < REPEAT_NUM; repeat++){
      ...
      if(rank == root){
        for(i = 0; i < steps; i++){
          MPI_Send(buf, msg_size[size_seq], MPI_CHAR, Intpow(2, i), tag, MPI_COMM_WORLD);
        }
      }
      else{
        MPI_Recv(buf, msg_size[size_seq], MPI_CHAR, mysender, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        for(i = ((int)floor(log2f((float)rank))+1); (Intpow(2, i) + rank) < proc_size; i++){
          MPI_Send(buf, msg_size[size_seq], MPI_CHAR, Intpow(2, i)+rank, tag, MPI_COMM_WORLD);
        }
      }
      t2 = MPI_Wtime();
      t[size_seq] += t2 - t1; 
